...
const UserDot = ({
  ...
  return (
    ...
    >
      ...
      >
        <View
          className="w-5 h-5 rounded-full bg-white border-2 border-[#ED5D55]"
          style={{
            shadowColor: "#000",
            shadowOpacity: 0.3,
            shadowRadius: 4,
    ...
    >
      <TouchableWithoutFeedback onPress={onClose}>
        ...
        >
          <TouchableWithoutFeedback>
            <View
              className="bg-[#FDFBF7] w-full max-w-sm rounded-3xl p-6 items-center"
              style={{
                shadowColor: "#000",
                shadowOpacity: 0.2,
                shadowRadius: 10,
            ...
            >
              ...
              <View className="flex-row w-full space-x-3">
                ...
                <TouchableOpacity
                  onPress={onSend}
                  className="flex-1 bg-[#FF5C8D] py-4 rounded-xl items-center"
                  style={{
                    shadowColor: "#FF5C8D",
                    shadowOpacity: 0.3,
                    shadowRadius: 5,
    ...
    <View className="flex-1">
      ...
      <SafeAreaView className="flex-1">
        ...
        <View className="flex-row justify-between items-center px-6 pt-4 mb-4 z-50">
          {isVisible ? (
            <View
              className="flex-row rounded-full p-1 h-12 items-center"
              style={{ backgroundColor: "rgba(0,0,0,0.1)" }}
            >
              <TouchableOpacity
                onPress={() => setViewMode("signals")}
                className={`h-10 w-12 items-center justify-center rounded-full ${
                  viewMode === "signals" ? "bg-white" : "bg-transparent"
                }`}
              >
                <Ionicons
                  name="heart"
                  size={20}
                  color={viewMode === "signals" ? "#ED5D55" : "white"}
                />
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => setViewMode("nearby")}
                className={`h-10 w-12 items-center justify-center rounded-full ${
                  viewMode === "nearby" ? "bg-white" : "bg-transparent"
                }`}
              >
                <Ionicons
                  name="people"
                  size={22}
                  color={viewMode === "nearby" ? "#ED5D55" : "white"}
                />
              </TouchableOpacity>
            </View>
          ) : (
          ...
          <TouchableOpacity
            ...
            className="w-12 h-12 rounded-full items-center justify-center border border-white/30"
          ...
          {isVisible ? (
            ...
            >
              ...
              <Text className="text-white/80 text-center px-10 leading-6">
                Your radar is off. You are not visible to others and cannot see
                nearby signals.
              </Text>
              <TouchableOpacity
                onPress={() => router.push("/settings")}
              ...
              >
                <Text className="text-[#ED5D55] font-bold">
                  Enable in Settings
                </Text>
              </TouchableOpacity>
            </View>
